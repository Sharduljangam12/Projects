...
Create an abstract base class called Readable:

    Inherit from ABC.

    Define an abstract method read().

Update the Book and ReferenceBook classes to inherit from Readable and implement the read() method:

    Book.read() ➜ return "Reading book: <title>"

    ReferenceBook.read() ➜ return "Reading reference book (inside library only): <title>"

Test abstraction by calling read() via a loop:
